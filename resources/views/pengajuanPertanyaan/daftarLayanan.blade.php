...
<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between">
        ...
        <h3 class="card-title"><strong> Daftar Pertanyaan Layanan </strong></h3>
    </div>
    <div class="card-body">
        @php
        ...
        button.addEventListener('click', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`{{ url('pengajuanPertanyaan/layanan/setujui') }}/${pertanyaanId}`, {
                        method: 'POST',
                        headers: {
    ...
    document.querySelectorAll('.hapus-pertanyaan').forEach(button => {
        button.addEventListener('click', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`{{ url('pengajuanPertanyaan/layanan/hapus') }}/${id}`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json',
    ...
    document.querySelectorAll('.tolak-pertanyaan').forEach(button => {
        button.addEventListener('click', function () {
            ...
            `,
                ...
                if (result.isConfirmed) {
                    fetch(`{{ url('pengajuanPertanyaan/layanan/tolak') }}/${id}`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json',
